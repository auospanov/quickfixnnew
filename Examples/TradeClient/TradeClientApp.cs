...
GO
        ...
        private void PersistInstrsSignalRMessages(IReadOnlyList<SignalRQuoteUpdateDto> allInstrsUpdates)
        {
            ...
            if (allInstrsUpdates == null || allInstrsUpdates.Count == 0)
                return;

            try
            {
                BulkInsertSignalRMessagesInstrs(allInstrsUpdates);
                recToLog($"signalRMessages INSTRS записано: {allInstrsUpdates.Count}");
            }
            catch (Exception ex)
            {
        ...
        private void ProcessQuotesSignalRBranch(List<quotesSimple> tempList, List<GlobalGlassContainer> glassBatch)
        {
            ...
            var allSnapshotItems = lastSnapshotByInstrument.Values
                .Concat(bidAskSnapshotByInstrument.Values)
                .ToList();
            var allInstrsUpdates = lastUpdates.Concat(bidAskUpdates).ToList();
            ...
            try
            {
                if (marketDbReady)
                {
                    try
                    {
                        FixDataUpdateSnapshot(JsonConvert.SerializeObject(allSnapshotItems));
                    }
                    catch (Exception ex)
                    {
                    ...
                    PersistInstrsSignalRMessages(allInstrsUpdates);
                }
                ...
                if (endpoints.Count > 0 && allInstrsUpdates.Count > 0)
                    _ = SendInstrsSignalRBatchAsync(allInstrsUpdates, endpoints);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[TimerTick] INSTRS branch error: {ex.Message}");
            ...
            else if (message.Header.GetString(QuickFix.Fields.Tags.MsgType) == QuickFix.Fields.MsgType.LOGOUT)
            {
                try
                {
                    ...
                    if (message.IsSetField(QuickFix.Fields.Tags.Text))
                    {
                        ...
                        //AIX SP2
                        // первый вариант: Lower sequence received than expected without PossDup flag. Expected/Received = 2943/1
                        // Нужно брать "Expected 2943" - это то, что сервер ожидает получить
                        // второй вариант: MsgSeqNum too low, expecting 675 but received 59

                        //KASE
                        //The incoming message has a sequence number (1) less than expected (13) and the PossDupFlag is not set. This indicates a serious error. [RefSeqNum: 1, RefMsgType: A]

                        if ((text.Contains("sequence", StringComparison.OrdinalIgnoreCase) && text.Contains("Expected", StringComparison.OrdinalIgnoreCase))
                            || text.Contains("MsgSeqNum too low", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            {
                                // НОВЫЙ ФОРМАТ:
                                // "The incoming message has a sequence number (2) less than expected (13)"
                                var bracketMatch = System.Text.RegularExpressions.Regex.Match(
                                    text,
                                    @"less\s+than\s+expected\s*\((\d+)\)",
                                    System.Text.RegularExpressions.RegexOptions.IgnoreCase);

                                if (bracketMatch.Success && bracketMatch.Groups.Count > 1)
                                {
                                    if (ulong.TryParse(bracketMatch.Groups[1].Value, out ulong expectedSeqNum))
                                    {
                                        var session = Session.LookupSession(sessionId);
                                        if (session != null)
                                        {
                                ...
                                {
                                    // СУЩЕСТВУЮЩИЙ fallback — НЕ МЕНЯЛ
    ...
    public byte? IsReal { get; set; }
        ...
        private void WriteOrderToAIS(QuickFix.Message m)
        {
            try
            {
                ...
                if (m.IsSetField(54)) order.Side = GetSideName(m.GetChar(54));
                        ...
                        for (int i = 1; i <= sidesCount; i++)
                        {
                            ...
                            if (tcr.IsSetNoSides())
                            {
                                ...
                                for (int i1 = 1; i1 <= count; i1++)
                                {
                                    ...
                                    if (group.IsSetOrderID())
                                        tc.OrderID = group.OrderID.Value;

                                    if (group.IsSetClOrdID())
                                        tc.ClOrdID = group.ClOrdID.Value;

                                    if (group.IsSetSecondaryClOrdID())
